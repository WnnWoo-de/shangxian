  ...
  try {
    ...
    const payload = {
      ...
      items: itemRows.map(row => ({
        ...
        weightInput: row.quantityInput,
        quantity: parseWeightExpression(row.quantityInput ?? row.quantity),
        totalWeight: parseWeightExpression(row.quantityInput ?? row.quantity),
        unitPrice: Number(row.unitPrice || 0),
        amount: multiplyMoney(parseWeightExpression(row.quantityInput ?? row.quantity), Number(row.unitPrice || 0)),
        source: row.source || '',
